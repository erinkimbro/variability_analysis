...
#get files
PATH = '../../..'
PATH2 = '../../../results_stacked_'+args.filter+'/'+ str(int(args.field)) + '_' + str(int(args.ccdid)) + '_' + str(int(args.qid))
...
for z in range(len(dir)):
    ...
    hdul = fits.open(PATH + '/workspace/data/Merian_DR1_photoz_EAZY_v3.0.fits')
    ...
    if (len(tbl4_gh7) > 0) & ((len(tbl4_nsa)>0) or (len(tbl4_merian)>0)) :
        tbl4_gh7.add_column(np.repeat('misc', len(tbl4_gh7)), name='catalog')
        tbl4 = vstack([tbl4, tbl4_gh7])

    if (len(tbl4_gh7) > 0) & ((len(tbl4_nsa)==0) and (len(tbl4_merian)==0)) :
        tbl4_gh7.add_column(np.repeat('misc', len(tbl4_gh7)), name='catalog')
        tbl4 =  tbl4_gh7
    ...
    if len(tbl4)>0:
        ...
        if len(np.array(name)[merian_mask])>0:
            tpl_t_m = Table([np.array(name)[merian_mask], catalog_matches_merian['objectId_Merian'], pos.ra.value[merian_mask], pos.dec.value[merian_mask], catalog_matches_merian['logmass_gaap'], 
                    catalog_matches_merian['logsfr_gaap'], catalog_matches_merian['z_ml'], avg_mag[merian_mask], avg_mag_err[merian_mask], avg_std[merian_mask],
        ...
        with open('../../../results_stacked/field_log.txt','a') as f:
